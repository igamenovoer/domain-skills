...
1. Precompiled vendor/runtime libraries
   cuBLAS, cuBLASLt, cuDNN, CUB, NCCL, framework/runtime kernels

2. Header/template or generated-kernel libraries
   CUTLASS/CuTe, CUTLASS DSL, TileLang/CuTile-style generators,
   domain wrappers such as DeepGEMM or FlashInfer

3. Handcrafted CUDA kernels
...
| Scan, prefix sum, reduction, histogram, sort, select, compaction | CUB or framework primitives | Do not hand-roll common routing plumbing until CUB shape, temporary storage, and stream constraints are checked. |
...
- the required build, link, or runtime-loading path is unavailable
...
- Treat library availability as a build and binding fact, not an assumption. Verify headers, linked libraries, runtime loading, handle lifetime, stream use, workspace, and architecture flags before recommending cuBLASLt, cuDNN, CUB, NCCL, CUTLASS/CuTe, or generated kernels as the primary experiment.
