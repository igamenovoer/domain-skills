...
- Add `metadata.skill_invocation_notation` to every created Imsight skill or subskill entrypoint, and to any other page that uses object-style invocation designators. Use the standard value from `references/imsight-skill-style-guide.md` and keep that generic notation text out of Markdown bodies.
...
10. Confirm every skill and subskill entrypoint declares `metadata.skill_invocation_notation`; if any other page uses object-style invocation designators such as `X->Y`, `X->Y->cmd()`, or `X->parent()->child()`, confirm it declares the same metadata. Confirm the generic notation text does not appear in Markdown bodies.
