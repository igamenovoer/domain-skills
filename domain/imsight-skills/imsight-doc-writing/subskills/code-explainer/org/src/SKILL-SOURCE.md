...
description: Use when a user asks for a source-code explainer, annotated implementation page, literate-programming view, or labml-style page that aligns prose, equations, data shapes, and exact source lines from a code file. Do not use for ordinary code review or documentation without a webpage deliverable.
metadata:
...
2. **Understand the implementation**. Read the complete source and the minimum supporting code, tests, types, or upstream references needed to explain it accurately.
...
4. **Choose the page integration**. Extend the user's existing web stack when one is in scope; otherwise create a small static page without an unnecessary framework.
...
7. **Deliver the result**. Report the page entrypoint, supporting files, source snapshot, validation performed, and any intentionally omitted source ranges.
