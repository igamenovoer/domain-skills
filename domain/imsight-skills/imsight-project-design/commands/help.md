...
2. **List public subcommands**. Include `help`, `scaffold`, `define-feature`, `design-usecase`, `design-interface`, `design-skill`, `design-gates`, `plan-feature`, `design-agent-task`, and `manual-refine` with one-line descriptions. Mention that `design-skill` is for agent-skill features, `design-interface` is for non-skill interfaces and contracts, `design-gates` writes an optional ordered verification-gate ladder as planning-stage specs, `plan-feature` writes a high-level milestone plan describing what each milestone must enable, and `manual-refine` records user-directed decisions as ADRs and propagates them into affected artifacts.
