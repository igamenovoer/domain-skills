...
- [`imsight-dev-box-init`](imsight-dev-box-init/SKILL.md): Development host setup and installation index for Imsight-preferred tooling. It covers software installs, development packages, CLI tooling, Houmao tooling, Tavily setup, Claude Code launchers such as `claude-kimi` and `claude-gac`, and related dev-box bootstrap work.
...
- [`imsight-mentality-mgr`](imsight-mentality-mgr/SKILL.md): Manage mentality catalogs and project or agent-memory selections; configure Ponytail intensity and edit scope, recall effective settings, and run self-contained Brooks or Ponytail reviews.
- [`imsight-paper-reading`](imsight-paper-reading/SKILL.md): Read an academic paper into a structured, citation-grounded reading note, optionally cross-referencing explicitly provided or repository-resident reference source code.
...
- [`imsight-project-mgr`](imsight-project-mgr/SKILL.md): Manual-or-routed project manager for Pixi/Python initialization and structure, on-demand bootstrap-managed external resources, project rules, clean worktrees, isolated implementation, and GitHub releases.
