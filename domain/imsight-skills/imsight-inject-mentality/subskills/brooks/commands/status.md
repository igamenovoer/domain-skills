...
Rules artifact: <brooks-rules.md path when project-persisted>
```

When state is unset, explain that the built-in retained selection is all rules but Brooks remains disabled. When project persistence is in effect, also name `AGENTS.md` and the referenced Brooks rules artifact.
