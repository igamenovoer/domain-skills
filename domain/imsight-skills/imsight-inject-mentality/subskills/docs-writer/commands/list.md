...
1. Resolve Docs Writer state from `../references/state.md` without mutating it.
...
4. Report the writing rules, selector groups, enabled state, persistence destination, and both project files when project persistence applies.
...
Prefer a compact table with code, canonical name, selected state, and effective state. A selected rule is effective only while Docs Writer is enabled and applicable to the current task.
