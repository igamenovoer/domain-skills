...
2. Ask each child for active state and current-task applicability without inspecting its private state schema.
3. Request a compact rendering only from children that are both active and applicable.
...
The parent may add headings and remove exact duplicate lines, but it does not rewrite a child's meaning or infer guidance for an inactive child.
...
3. The compatible subset of active mentality guidance.
...
- DO NOT activate, deactivate, or reconfigure a mentality while composing it.
- DO NOT include inactive or inapplicable mentality guidance.
