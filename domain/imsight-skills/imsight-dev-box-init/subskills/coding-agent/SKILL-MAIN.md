...
| `create-kimi-manager` | Deploy or repair the `kimi-project.sh` Kimi Code account manager: plant independent OAuth auth sessions (identified per login by `(user_id, device_id)`, so same-account logins stay distinct) from isolated homes into project-scope homes, manage session aliases and private slots, detect re-login drift, audit per-session credential freshness, rescue fresher copies back to canonical slot homes, return sessions home on `undeploy`, and back up or restore a home's auth state. | `references/create-kimi-manager.md` |
