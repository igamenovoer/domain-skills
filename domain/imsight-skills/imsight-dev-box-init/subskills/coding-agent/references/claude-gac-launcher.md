...
Use this reference only for a `claude-gac` launcher that targets `https://gaccode.com/claudecode`. The endpoint and user-provided GAC API key are embedded directly in the generated Linux launcher or Windows PowerShell profile block. Claude Code JSON files remain unchanged.
...
2. Re-check Claude Code's current environment-variable and permission-flag behavior plus GAC's current endpoint and model-discovery behavior. Treat the contract below as authoritative for this skill, but adapt version-sensitive syntax when current evidence requires it.
3. Obtain the GAC API key under **Required Input**; stop instead of generating a placeholder or partial launcher when the key is unavailable.
4. Implement every invariant in **GAC Launcher Contract** using an OS-native launcher. The bundled scripts under **Reference Implementations** are worked examples, not mandatory entrypoints.
5. Run **Verification** against the resulting launcher, including environment scoping, argument forwarding, model discovery, permission mode, and exit-code preservation.
6. Report the launcher location, implementation choices, and validation results without printing the key.
...
`claude-gac` runtime arguments are Claude Code arguments. Both lanes prepend `--dangerously-skip-permissions` by default, inject no model or updater defaults, and then pass every user-supplied argument through unchanged. The generation-time permission-prompting option omits only the permissive flag.
...
test "$(stat -c '%a' "$HOME/.local/bin/claude-gac")" = 700
grep -q '^export ANTHROPIC_BASE_URL=https://gaccode.com/claudecode$' "$HOME/.local/bin/claude-gac"
grep -q '^export ANTHROPIC_API_KEY=' "$HOME/.local/bin/claude-gac"
grep -q "^export CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY='1'$" "$HOME/.local/bin/claude-gac"
grep -q -- '--dangerously-skip-permissions' "$HOME/.local/bin/claude-gac"
! grep -Eq 'gac-api-key|claude-gac-token|GAC_BASE_URL|claude-gac-base-url' "$HOME/.local/bin/claude-gac"
claude-gac --version
```
...
Start a fresh `claude-gac` session and inspect `/status` and `/model`. The base URL must be `https://gaccode.com/claudecode`, and gateway-discovered rows should include `claude-fable-5` when the provided key's plan exposes it.
...
- If Linux cannot find `claude-gac`, confirm the generated file is executable and `$HOME/.local/bin` is on PATH.
