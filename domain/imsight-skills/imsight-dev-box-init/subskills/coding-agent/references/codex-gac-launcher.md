...
2. Obtain the GAC API key under **Required Input**; stop instead of writing a placeholder launcher when the key is unavailable.
3. Select the installed Codex version's profile layout under **Version and Profile Compatibility**.
4. Implement every invariant in **Codex-GAC Contract** using the dedicated provider profile and an OS-native launcher. Treat **Reference Implementations** as worked examples rather than mandatory machinery.
5. Run **Verification**, including profile loading, scoped authentication, argument forwarding, permission mode, GAC completion, and a plain official Codex completion.
6. Report the profile and launcher locations, Codex version, selected model, permission mode, and validation results without printing the key.
...
| Provider isolation | Put GAC selection and provider settings in a dedicated `gac` profile layer rather than the base `config.toml`. |
...
On Codex CLI 0.154.0, `--profile gac` layers `$CODEX_HOME/gac.config.toml` on top of the base user config. With the default home, that file is `~/.codex/gac.config.toml`. The same version rejects legacy `[profiles.gac]` content in `config.toml` and instructs the user to move it to the separate profile file.
...
Resolve the active Codex home from `CODEX_HOME` when it is intentionally set; otherwise use `~/.codex`. Create `<codex-home>/gac.config.toml` without changing `<codex-home>/config.toml` or `<codex-home>/auth.json`:
...
| --- | --- | --- | --- |
| Linux or macOS | Executable `~/.local/bin/codex-gac` Bash script | Exported only in the child launcher process | `~/.codex/gac.config.toml`, or the intentionally selected `CODEX_HOME` |
| Windows PowerShell | Managed `codex-gac` function in `$PROFILE.CurrentUserCurrentHost` | Saved, set for invocation, then restored in `finally` | `$HOME\.codex\gac.config.toml`, or the intentionally selected `CODEX_HOME` |
...
Create `~/.local/bin/codex-gac` with the real key substituted only in the local file:
...
if [[ -z "$codex_bin" ]]; then
  echo "codex-gac: codex binary not found" >&2
  exit 127
fi

exec "$codex_bin" --profile gac --dangerously-bypass-approvals-and-sandbox "$@"
```

Then restrict the credential-bearing file and make sure its directory is on `PATH`:

```bash
chmod 0700 "$HOME/.local/bin/codex-gac"
```
...
function codex-gac {
    ...
    try {
        ...
        & $codexCommand --profile gac --dangerously-bypass-approvals-and-sandbox @args
...
Examples that must continue to work include `codex-gac`, `codex-gac exec "Reply with exactly: GAC_OK"`, `codex-gac --version`, and any current Codex subcommand or option supported by the installed version.
...
```bash
bash -n "$HOME/.local/bin/codex-gac"
test -x "$HOME/.local/bin/codex-gac"
test "$(stat -c '%a' "$HOME/.local/bin/codex-gac")" = 700
rg -n 'model_provider|model =|base_url|wire_api|env_key|requires_openai_auth' "$HOME/.codex/gac.config.toml"
rg -q 'GAC_API_KEY=' "$HOME/.local/bin/codex-gac"
rg -q -- '--profile gac' "$HOME/.local/bin/codex-gac"
rg -q -- '--dangerously-bypass-approvals-and-sandbox' "$HOME/.local/bin/codex-gac"
command -v codex-gac
```
...
([regex]::Matches($profileText, [regex]::Escape('# >>> codex-gac launcher >>>'))).Count -eq 1
([regex]::Matches($profileText, [regex]::Escape('# <<< codex-gac launcher <<<'))).Count -eq 1
$profileText.Contains("--profile gac")
$profileText.Contains("--dangerously-bypass-approvals-and-sandbox")
$profileText.Contains("GAC_API_KEY")
Get-Command codex-gac -CommandType Function
Select-String -Path "$HOME\.codex\gac.config.toml" -Pattern 'model_provider|model =|base_url|wire_api|env_key|requires_openai_auth'
```
...
```text
codex-gac exec "Reply with exactly: GAC_OK"
...
- If `--profile gac` reports legacy profile configuration, move the GAC keys out of `[profiles.gac]` and into the version-appropriate separate profile file without changing unrelated base settings.
...
- If Codex asks for official login during `codex-gac`, verify `requires_openai_auth = false`, `env_key = "GAC_API_KEY"`, and the launcher's scoped key assignment.
...
- If Linux cannot find `codex-gac`, confirm mode `0700` and that `~/.local/bin` is on `PATH`.
- If Windows cannot find `codex-gac`, reload the same PowerShell profile that was edited and compare its path with `$PROFILE.CurrentUserCurrentHost`.
...
- DO NOT use a legacy `[profiles.gac]` table when the installed Codex version requires profile-v2 files.
