...
3. Generate the launcher with the bundled script under **Create The Launcher**, applying **Defaults** for any value the user did not specify.
...
Confirm isolation by inspecting the generated launcher with redaction-safe patterns:

```bash
rg -n 'KIMI_CODE_HOME|KIMI_HOME_OVERRIDE|exec' "$HOME/.local/bin/kimi-<suffix>"
```
...
- Launcher runtime arguments are Kimi Code CLI arguments. The generated launcher passes every argument through unchanged and must not consume, rename, reorder, or reinterpret `kimi` flags.
