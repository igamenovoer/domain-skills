...
description: Use when explicitly invoking imsight-dev-box-init, routing from another Imsight skill, or using Imsight context to configure Codex, Claude Code, Antigravity CLI, Kimi Code CLI, Houmao, Hermes, Hindsight, Feishu, Ponytail, Tavily, Context7, BaiduPCS-Go, or related development-host tooling. Do not use for generic setup tasks without Imsight context.
metadata:
...
- DO NOT place credentials in skill sources, generated documentation, logs, or tracked files; keep any contract-authorized embedding confined to the user's local credential-bearing launcher.
